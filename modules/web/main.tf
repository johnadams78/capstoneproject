...
$fm=isset($_GET['make'])?$_GET['make']:'';$ft=isset($_GET['type'])?$_GET['type']:'';$fc=isset($_GET['category'])?$_GET['category']:'';$pmin=(int)(isset($_GET['min_price'])?$_GET['min_price']:0);$pmax=(int)(isset($_GET['max_price'])?$_GET['max_price']:999999);$sort=isset($_GET['sort'])?$_GET['sort']:'price_desc';
$filtered=array_filter($cars,function($c)use($fm,$ft,$fc,$pmin,$pmax){return(!$fm||$c[0]==$fm)&&(!$ft||$c[5]==$ft)&&(!$fc||$c[4]==$fc)&&$c[3]>=$pmin&&$c[3]<=$pmax;});
usort($filtered,function($a,$b)use($sort){if($sort=='price_asc')return $a[3]-$b[3];if($sort=='hp_desc')return $b[7]-$a[7];if($sort=='name_asc')return strcmp($a[0],$b[0]);return $b[3]-$a[3];});
$id=@file_get_contents('http://169.254.169.254/latest/meta-data/instance-id');if(!$id)$id='N/A';$az=@file_get_contents('http://169.254.169.254/latest/meta-data/placement/availability-zone');if(!$az)$az='N/A';

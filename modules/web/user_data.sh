...
$makeList=array();$r=$conn->query("SELECT DISTINCT make FROM cars ORDER BY make");while($row=$r->fetch_assoc())$makeList[]=$row['make'];
$typeList=array();$r=$conn->query("SELECT DISTINCT type FROM cars ORDER BY type");while($row=$r->fetch_assoc())$typeList[]=$row['type'];
$catList=array();$r=$conn->query("SELECT DISTINCT category FROM cars ORDER BY category");while($row=$r->fetch_assoc())$catList[]=$row['category'];
...
<select name="type"><option value="">All Types</option><?php foreach($typeList as $t):?><option value="<?=$t?>"<?=$ft==$t?' selected':''?>><?=$t?></option><?php endforeach;?></select>
